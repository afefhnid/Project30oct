...
Corpus_new = pd.concat([positif, negatif], ignore_index=True)[
    ['review', 'label']]
...
def model(phrase):
    ...
    # decode_error="replace", vocabulary=pickle.load(open("reg_age_model.pkl", "rb")))
    # user = transformer.fit_transform(
    # loaded_vec.fit_transform([nettoyage(phrase)]))

    cls = pickle.load(open("reg_age_model.pkl.pkl", "rb"))
